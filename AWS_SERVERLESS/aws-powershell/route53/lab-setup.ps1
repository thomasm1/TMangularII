...
$amiRegionA = "ami-48351d32" #aws-elasticbeanstalk-amzn-2017.09.1.x86_64-ecs-hvm-201801192255 (ami-48351d32)
$AWSRegionB = "us-west-1"  #$amiRegionB = "ami-f7383a97" #aws-elasticbeanstalk-amzn-2017.09.1.x86_64-ecs-hvm-201801180451
$amiRegionB = "ami-1fe0e47f" #aws-elasticbeanstalk-amzn-2017.09.1.x86_64-ecs-hvm-201712150350 (ami-1fe0e47f)

# Search for AMIs by name
$amiName = "aws-elasticbeanstalk-amzn-2017.09.1.x86_64-ecs-hvm-2018011*"
#Get-EC2ImageByName -Region "EU-west-2" -Name $amiName | ft -Property Name,ImageId     ### /// <-comment line out after setup .. 

# Set your IP subnet for SSH access
$myIP = "10.65.102.5" #"24.96.154.168/29"                         ## /// <--set IP from which you want secure ...[] us.syncsort.com]

# Set the name of your SSH keypair
$regionAKeyname = "dns-e-dailytechfilter" # "ccnetkeypair"
$regionBKeyname = "dns-dailytechfilter" # "ccnetkeypair"

# Get existing keypairs
Get-EC2KeyPair -Region "us-east-1" # dns-e-dailytechfilter
Get-EC2KeyPair -Region "us-west-1" # dns-dailytechfilter
